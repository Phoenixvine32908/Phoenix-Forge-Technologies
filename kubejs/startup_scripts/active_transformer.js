 ...
 */
const ActiveTransformerMachine = Java.loadClass(
  "com.gregtechceu.gtceu.common.machine.multiblock.electric.ActiveTransformerMachine",
);
const GTMemoizer = Java.loadClass(
  "com.gregtechceu.gtceu.utils.memoization.GTMemoizer",
);

// Function<MultiblockMachineDefinition, BlockPattern>
const active_transformer_pattern = (definition) =>
  FactoryBlockPattern.start()
    ...
    .where(
      ...
      Predicates.controller(
        Predicates.blocks(GTMultiMachines.ACTIVE_TRANSFORMER.get()),
      ),
    )
    .where(
      "X",
...
StartupEvents.postInit((event) => {
  GTMultiMachines.ACTIVE_TRANSFORMER.setPatternFactory(
    GTMemoizer.memoize(() =>
      active_transformer_pattern.apply(GTMultiMachines.ACTIVE_TRANSFORMER),
    ),
  );
});
