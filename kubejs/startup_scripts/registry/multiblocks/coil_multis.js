const CoilWorkableElectricMultiblockMachine = Java.loadClass(
...
GTCEuStartupEvents.registry("gtceu:machine", (event) => {
  const GCYMRecipeTypes = Java.loadClass(
    "com.gregtechceu.gtceu.common.data.GCYMRecipeTypes",
  ); // Have to load it here because ALLOY_BLAST_RECIPES is not defined outside of the event
  event
    .create("emberwake_alloy_hearth", "multiblock")
  ...
  event
    .create("advanced_cracking_unit", "multiblock")
    ...
    .recipeModifiers([
      ...
      GTRecipeModifiers.OC_NON_PERFECT_SUBTICK,
      GTRecipeModifiers.BATCH_MODE,
      (machine, recipe) => GTRecipeModifiers.crackerOverclock(machine, recipe),
    ])
    ...
    .pattern((definition) =>
      FactoryBlockPattern.start()
        ...
        .where(
          "B",
          Predicates.blocks("gtceu:tungstensteel_turbine_casing")
            .setMinGlobalLimited(10)
            .or(Predicates.abilities(PartAbility.MAINTENANCE).setExactLimit(1))
            .or(
              Predicates.abilities(
        ...
        .where(
          "C",
          Predicates.blocks("gtceu:void_touched_tungsten_steel_frame"),
        )
  ...
  event
    .create("superheated_pyrolyzing_oven", "multiblock")
    .rotationState(RotationState.NON_Y_AXIS)
    .machine((holder) => new CoilWorkableElectricMultiblockMachine(holder))
