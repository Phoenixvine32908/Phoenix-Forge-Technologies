...
GTCEuStartupEvents.registry("gtceu:machine", event => {
    ...
    event.create("large_steam_sifter", "multiblock")
        .machine((holder) => new $SteamMulti(holder, 4))
        .rotationState(RotationState.NON_Y_AXIS)
        .recipeType("sifter")
        .recipeModifier((machine, recipe) => $SteamMulti.recipeModifier(machine, recipe), true)
        .appearanceBlock(GTBlocks.CASING_PRIMITIVE_BRICKS)
        .pattern(definition => FactoryBlockPattern.start()
            .aisle("AAAAA", "BCBCB", "BBBBB", "BBBBB")
            .aisle("ADDDA", "CCFCC", "BCDCB", "BBDBB")
            .aisle("ADDDA", "BDEDB", "BDBDB", "BDBDB")
            .aisle("ADDDA", "CCDCC", "BCDCB", "BBDBB")
            .aisle("AAAAA", "BCBCB", "BBBBB", "BBBBB")
            .where("A", Predicates.blocks("gtceu:industrial_steam_casing"))
            .where("B", Predicates.blocks("minecraft:air"))
            .where("C", Predicates.blocks("phoenixcore:fiery_bronze_frame"))
            .where("D", Predicates.blocks("gtceu:steam_machine_casing").setMinGlobalLimited(4) // Corrected line
                .or(Predicates.abilities(PartAbility.STEAM_IMPORT_ITEMS).setMaxGlobalLimited(1))
                .or(Predicates.abilities(PartAbility.STEAM).setMaxGlobalLimited(1))
