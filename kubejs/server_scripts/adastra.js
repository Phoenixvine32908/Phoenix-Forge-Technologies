ServerEvents.recipes((event) => {
  ...
  event.replaceInput(
    ...
    "gtceu:uranium_236_frame",
  );
  event.replaceInput(
    { id: "gtceu:shaped/electric_motor_lv_iron" },
    "gtceu:iron_rod",
    "gtceu:aurum_steel_rod",
  );
  event.replaceInput(
    { id: "gtceu:assembler/casing_zpm" },
    "gtceu:naquadah_alloy_plate",
    "gtceu:advanced_quin_naquadian_alloy_plate",
  );
  event.replaceInput(
    { id: "gtceu:shaped/casing_zpm" },
    "gtceu:naquadah_alloy_plate",
    "gtceu:advanced_quin_naquadian_alloy_plate",
  );
  event.remove({ id: /nasa_workbench/ });
  const rocket = [
    // Plate mats currently just based on color of rocket for testing
...
ServerEvents.recipes((event) => {
  ...
  for (const tier in casingReplacements) {
    ...
    event.replaceInput(
      { id: /^gtceu:.*/ },
      replacement.oldPlate,
      replacement.newPlate,
    );
  }
  ...
  for (const tier in rotorReplacements) {
    ...
    event.replaceInput(
      { id: /^gtceu:.*/ },
      replacement.oldRotor,
      replacement.newRotor,
    );
  }
});
