...
function addWannabeeRecipe(beeName, amberItemWithEntityType, expectedMobType, outputs) {
  const input = Item.of('productivebees:bee_cage', `{type:"productivebees:${beeName}", entity: "productivebees:configurable_bee"}`);
  const outputItem = Item.of(`productivebees:configurable_comb`, `{EntityTag:{type:"productivebees:${beeName}"}}`);
  // The wannabeeInput now only checks for the entityType within the BlockEntityTag
  const wannabeeInput = amberItemWithEntityType;
  makeMatrixRecipes(`kubejs:gtceu/melferious_matrix/${beeName}_wannabee`, input, wannabeeInput, outputs); // Use wannabeeInput
}

// Example WannaBee - Using the amber block and only checking entityType
const amberedCreeperItem = Item.of('productivebees:amber', { EntityData: { entityType: "minecraft:creeper" } })

addWannabeeRecipe(
  'wannabee_creeper', // Give this wannabee recipe a unique name
  amberedCreeperItem, // Pass the Item.of with the simplified NBT
  'minecraft:creeper', // The expected mob type inside the amber
  [
    { item: 'minecraft:gunpowder', chance: 8000 }, // Example Creeper drop
    // Add other desired drops for a Creeper wannabee
  ]
);

// Another example for a Zombie in amber
const amberedZombieItem = Item.of('productivebees:amber', `{BlockEntityTag:{EntityData:{entityType:"minecraft:zombie"}}}`);
...
addWannabeeRecipe(
  ...
  'minecraft:zombie',
  [
    { item: 'minecraft:rotten_flesh', chance: 8000 },
    { item: 'minecraft:carrot', chance: 1500 },
    { item: 'minecraft:potato', chance: 1500 }
  ]
);
